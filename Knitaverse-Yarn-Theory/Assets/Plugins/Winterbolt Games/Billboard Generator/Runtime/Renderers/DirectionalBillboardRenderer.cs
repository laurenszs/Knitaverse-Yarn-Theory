...
namespace WinterboltGames.BillboardGenerator.Runtime.Renderers
{
	...
	public sealed class DirectionalBillboardRenderer : BillboardRendererBase
	{
		...
		public override bool IsValid(bool warn = true)
		{
			if (useAlternativeWorkflow)
			{
				if (meshFilter == null)
				{
					if (warn)
					{
						Debug.LogWarning($"{name} has no {nameof(meshFilter)} assigned.", this);
					}
				...
				if (meshRenderer == null)
				{
					if (warn)
					{
						Debug.LogWarning($"{name} has no {nameof(meshRenderer)} assigned.", this);
					}
				...
				else if (meshRenderer.sharedMaterials.Length < 1)
				{
					if (warn)
					{
						Debug.LogWarning($"{name} has a {nameof(meshRenderer)} assigned but with no materials.", this);
					}
		...
		public override void UpdateBillboard()
		{
			...
			if (useAlternativeWorkflow)
			{
				cachedTransform.localScale = GetScale(false);
			}
