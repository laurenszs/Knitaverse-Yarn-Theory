	...
	public sealed class MaxRectsBin
	{
		...
		private int ContactPointScoreNode(int x, int y, int width, int height)
		{
			...
			foreach (Rect usedRect in _usedRectangles)
			{
				...
				{
					score += CommonIntervalLength((int)usedRect.y, (int)usedRect.y + (int)usedRect.height, y, y + height);
				}

				if (usedRect.y == y + height || usedRect.y + usedRect.height == y)
				{
					score += CommonIntervalLength((int)usedRect.x, (int)usedRect.x + (int)usedRect.width, x, x + width);
				}
			}
