	...
	internal sealed class GeneratorSettingsAssetInspector : UnityEditor.Editor
	{
		public static void Draw(SerializedObject serializedObject)
		{
			...
			if (!useMainCameraProperty.boolValue)
			{
				...
				if (GUILayout.Button("Copy Main Camera Position"))
				{
					settingsProperty.FindPropertyRelative(nameof(GeneratorSettings.cameraOffset)).vector3Value = Camera.main.transform.position;
				}
			}
			else
			{
